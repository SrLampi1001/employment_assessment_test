...
| Frontend | **React 19.2 + TypeScript 5.7+ + Vite 8** | React 19 is the current major (Actions, `use`, Server Components stable); Vite 8 is current and requires Node 20.19+ / 22.12+ — pin **Node 24 (current Active LTS)** for the build stage. `react-infinite-scroll-component` (IntersectionObserver, ~4 kB gzipped, React 19 compatible) handles the lazy keyset history. | Angular if more familiar |
